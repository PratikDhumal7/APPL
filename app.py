...
def chembur():
    if 'loggedin' in session:
        cursor = mysql.connection.cursor()
        cursor.execute("select distinct(t_date) t_date from location inner join turf on lid=ltid inner join dates on tid=tdid where l_name='chembur' and t_name ='ACRES CLUB' and sta is null")
        vt1 = cursor.fetchall()
        # print(vt1)
        cursor = mysql.connection.cursor()
        cursor.execute("select distinct(t_date) t_date from location inner join turf on lid=ltid inner join dates on tid=tdid where l_name='chembur' and t_name ='CHEMBUR KARNATAKA' and sta is null")
        vt2 = cursor.fetchall()
...
def thane():
    if 'loggedin' in session:
        cursor = mysql.connection.cursor()
        cursor.execute("select distinct(t_date) t_date from location inner join turf on lid=ltid inner join dates on tid=tdid where l_name='thane' and t_name ='URBAN SPORTS' and sta is null")
        vt1 = cursor.fetchall()
        # print(vt1)
        cursor = mysql.connection.cursor()
        cursor.execute("select distinct(t_date) t_date from location inner join turf on lid=ltid inner join dates on tid=tdid where l_name='thane' and t_name ='BOUNCE FOOTBALL' and sta is null")
        vt2 = cursor.fetchall()
...
def vashi():
    if 'loggedin' in session:
        cursor = mysql.connection.cursor()
        cursor.execute("select distinct(t_date) t_date from location inner join turf on lid=ltid inner join dates on tid=tdid where l_name='vashi' and t_name ='NMSA' and sta is null")
        vt1 = cursor.fetchall()
        # print(vt1)
        cursor = mysql.connection.cursor()
        cursor.execute("select distinct(t_date) t_date from location inner join turf on lid=ltid inner join dates on tid=tdid where l_name='vashi' and t_name ='GOALBOX SAINATH' and sta is null")
        vt2 = cursor.fetchall()
...
def slot():
    ...
    if request.method == 'POST' and 't_date' in request.form and 't_name' in request.form and 'l_name' in request.form:
        ...
        cursor = mysql.connection.cursor()
        cursor.execute("select start_time,end_time,sta from location inner join turf on lid=ltid inner join dates on tid=tdid where l_name=%s and t_name =%s and t_date=%s and sta is null order by start_time ASC  ",(locat, turf, date,))
        dts = cursor.fetchall()
        # print(dts)
...
def booknow():
    if 'loggedin' in session:
        ...
        if request.method == 'POST' and 'b_date'in request.form and 'slot'in request.form and 'turf_name' in request.form  and 'locat' in request.form:
            ...
            cursor = mysql.connection.cursor()
            cursor.execute("insert into bookings(customer_name,cust_phone,locat,turf_name,b_date,starttime,endtime) values(%s,%s,%s,%s,%s,%s,%s)",(username,phone,locat,turf,date,starttime,endtime,))
            mysql.connection.commit()
            cursor.execute("update dates inner join turf on tid=tdid inner join location on lid=ltid set sta='booked' where l_name=%s and t_name=%s and t_date=%s and start_time=%s",(locat,turf,date,starttime))
            mysql.connection.commit()
            #print(booked)
